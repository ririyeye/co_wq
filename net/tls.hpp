...
template <lockable lock, template <class> class Reactor = epoll_reactor> class dtls_socket {
public:
    template <class D> using tp_base = two_phase_drain_awaiter<D, dtls_socket>;
        ...
        , _cbq(std::make_unique<callback_wq<lock>>(exec))
    {
        ...
        _ssl.reset(SSL_new(_ctx.native_handle()));
        if (!_ssl)
            throw std::runtime_error("SSL_new (dtls) failed");
        BIO* bio = BIO_new_dgram(_udp.native_handle(), BIO_NOCLOSE);
        if (!bio)
        ...
        if (mode == dtls_mode::Client)
            ...
            SSL_set_accept_state(_ssl.get());
            SSL_set_options(_ssl.get(), SSL_OP_COOKIE_EXCHANGE);
        }
        serial_queue_init(_send_q);
        serial_queue_init(_recv_q);
    }
    ...
    dtls_socket(dtls_socket&& other) noexcept
        : _exec(other._exec)
        , _reactor(other._reactor)
    ...
    dtls_socket& operator=(dtls_socket&& other) noexcept
    {
        if (this != &other) {
            close();
            _exec           = other._exec;
    ...
    lock&                      serial_lock() { return _io_serial_lock; }
    Reactor<lock>*             reactor() { return _reactor; }
    udp_socket<lock, Reactor>& underlying() { return _udp; }
    bool                       handshake_done() const noexcept { return _handshake_done; }
    SSL*                       ssl_handle() const noexcept { return _ssl.get(); }
    ...
    struct handshake_awaiter : io_waiter_base {
        ...
        explicit handshake_awaiter(dtls_socket& s) : owner(s) { }
            ...
            while (!owner._handshake_done) {
                ...
                if (ssl_err == SSL_ERROR_WANT_READ || ssl_err == SSL_ERROR_WANT_WRITE) {
                    ...
                    if (ssl_err == SSL_ERROR_WANT_WRITE)
                        mask |= EPOLLOUT;
                    this->func = &handshake_awaiter::resume_cb;
                    owner._reactor->add_waiter_custom(owner._udp.native_handle(), mask, this);
                    return;
                }
                result = detail::translate_failure(ssl_err);
                break;
    ...
    struct recv_awaiter : tp_base<recv_awaiter> {
        ...
        recv_awaiter(dtls_socket& s, void* b, size_t l, bool f)
            : tp_base<recv_awaiter>(s, s._recv_q), owner(s), buf(b), len(l), full(f)
        {
            this->route_ctx  = owner._cbq.get();
            this->route_post = &callback_wq<lock>::post_adapter;
        ...
        {
            uint32_t mask = self->wait_mask ? self->wait_mask : EPOLLIN;
            self->owner._reactor->add_waiter_custom(self->owner._udp.native_handle(), mask, self);
        }
    ...
    struct send_awaiter : tp_base<send_awaiter> {
        ...
        send_awaiter(dtls_socket& s, const void* b, size_t l, bool f)
            : tp_base<send_awaiter>(s, s._send_q), owner(s), buf(b), len(l), full(f)
        {
            this->route_ctx  = owner._cbq.get();
            this->route_post = &callback_wq<lock>::post_adapter;
