...
namespace detail {
    ...
    inline std::string build_http_response(int status_code, std::string_view reason, std::string_view body, std::string_view content_type = "text/plain; charset=utf-8")
    {
        std::string response;
...
namespace detail {
    inline std::string pick_subprotocol(const std::vector<std::string>& client_protocols, const std::vector<std::string>& server_protocols)
    {
...
Task<accept_result, Work_Promise<lock, accept_result>> accept(Socket& sock, const server_options& opts = {})
{
    ...
    if (parse_error || !ctx.message_complete) {
        std::string body = "WebSocket handshake failed: " + (parse_error_reason.empty() ? std::string("incomplete request") : parse_error_reason) + "\n";
    ...
    if (!upgrade_hdr || detail::to_lower(*upgrade_hdr) != "websocket" || !connection_hdr || !detail::connection_contains_upgrade(*connection_hdr) || !version_hdr || detail::trim(*version_hdr) != "13" || !key_hdr) {
        std::string response = detail::build_http_response(400, "Bad Request", "Missing or invalid WebSocket headers\n");
        (void)co_await sock.send_all(response.data(), response.size());
        if (opts.close_on_failure)
            sock.close();
        result.ok          = false;
    ...
    if (accept_key.empty()) {
        std::string response = detail::build_http_response(500, "Internal Server Error", "Failed to compute accept key\n");
        (void)co_await sock.send_all(response.data(), response.size());
        if (opts.close_on_failure)
            sock.close();
        result.ok          = false;
...
template <lockable lock, class Socket>
Task<void, Work_Promise<lock, void>> send_frame(Socket& sock, opcode op, std::span<const std::uint8_t> payload, const send_options& opts = {})
{
...
template <lockable lock, class Socket>
Task<message_result, Work_Promise<lock, message_result>> read_message(Socket& sock, const message_read_options& opts = {})
{
    message_result res;
    message        msg;
    ...
    for (;;) {
        ...
        if (fr.op == opcode::ping) {
            ...
            if (fr.payload.size() >= 2) {
                ...
                if (fr.payload.size() > 2)
                    msg.close_reason.assign(reinterpret_cast<const char*>(fr.payload.data() + 2), fr.payload.size() - 2);
            }
            res.ok    = true;
...
template <lockable lock, class Socket>
Task<void, Work_Promise<lock, void>> send_binary(Socket& sock, std::span<const std::uint8_t> data, const send_options& opts = {})
{
...
template <lockable lock, class Socket>
Task<void, Work_Promise<lock, void>> send_close(Socket& sock, std::uint16_t code = 1000, std::string_view reason = {}, const send_options& opts = {})
{
    std::vector<std::uint8_t> payload;
...
template <lockable lock, class Socket>
Task<void, Work_Promise<lock, void>> send_pong(Socket& sock, std::span<const std::uint8_t> payload = {}, const send_options& opts = {})
{
...
template <lockable lock, class Socket>
Task<void, Work_Promise<lock, void>> send_ping(Socket& sock, std::span<const std::uint8_t> payload = {}, const send_options& opts = {})
{
